...
RADIUS = 5
if RADIUS == 1:
    EXPERIMENT_NAME = 'unet_bce_loss'
else:
    EXPERIMENT_NAME = 'unet_bce_loss_radius'
    ...
    def gen_random_sample():
        ...
        while True:
            ...
            if (center_x + r_x < config.IMAGE_W and center_x - r_x > 0 and
                ...
                if RADIUS > 1:
                    mask = cv2.circle(mask, (int(center_x), int(center_y)), radius=int(RADIUS), color=1,  thickness=-1)
                else:
...
def binary_cross_entropy_loss(y_true, y_pred):
    ...
    loss = tf.reduce_mean(
        -tf.multiply(y_true, tf.log(cliped_y_pred)) +
...
def get_model():
    ...
    model.compile(optimizer=Adadelta(), loss=binary_cross_entropy_loss,
