...
# Pipeline: mask in range [0,1] as single pixel + sigmoid + weighted bce loss

EXPERIMENT_NAME = 'unet_weighted_bce_loss'
...
def weighted_binary_cross_entropy_loss(y_true, y_pred):
    eps = 1e-6
    cliped_y_pred = tf.clip_by_value(y_pred, eps, 1 - eps)
    class_weight = config.IMAGE_H * config.IMAGE_W - 1 # Assuming we have just 1 pixel per image
    loss = tf.reduce_mean(
        -tf.multiply(y_true, tf.log(cliped_y_pred)) * class_weight +
        -tf.multiply((1 - y_true), tf.log(1 - cliped_y_pred)))
    return loss
...
def all_zeros_baseline(y_true, y_pred):
    ...
    eps = 1e-6
    cliped_y_pred = tf.clip_by_value(y_pred_zeros, eps, 1 - eps)
    class_weight = config.IMAGE_H * config.IMAGE_W - 1 # Assuming we have just 1 pixel per image
    loss = tf.reduce_mean(
        -tf.multiply(y_true, tf.log(cliped_y_pred)) * class_weight +
        -tf.multiply((1 - y_true), tf.log(1 - cliped_y_pred)))
...
def get_model():
    ...
    model.compile(optimizer=Adadelta(), loss=weighted_binary_cross_entropy_loss,
